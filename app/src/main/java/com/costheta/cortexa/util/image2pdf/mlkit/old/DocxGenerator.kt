...
object DocxGenerator {
     ...
     */
    fun createDocxFromOcr(context: Context, pages: List<OcrPage>, fileName: File): Uri? {
        if (pages.isEmpty()) {
        ...
        try {
            pages.forEachIndexed { index, pageData ->
                ...
                textTitleRun.setText("Recognized Text (Language: ${pageData.detectedLanguage}):")
            ...
            if (docDir != null){
                val file = File(docDir, fileName.toString())
                FileOutputStream(file).use {
                    document.write(it)
                }
                Log.d(TAG, "DOCX saved successfully to ${file.absolutePath}")
